  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${siteUrl}/auth/callback?redirect=${encodeURIComponent(redirectPath)}`,
        },
      })
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-4">
      <div className="max-w-md w-full bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/60 p-8 sm:p-10 animate-in fade-in zoom-in-95">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-slate-500 text-xs mt-1">Sign in to manage your short links and analytics</p>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-slate-100 text-center text-xs text-slate-500">
          ...
          <Link href="/signup" className="font-semibold text-indigo-600 hover:text-indigo-700 underline">
            Create account
          </Link>
        </div>
      </div>
